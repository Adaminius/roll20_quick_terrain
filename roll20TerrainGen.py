...
COLORS_LIST = ["sky blue", "green yellow", "coral", "orchid", "DarkOliveGreen2", "plum3", "gray60", "wheat", "pale violet red", "salmon", "sienna", "lavender", "honeydew", "moccasin", "snow", "brown1", "green2", "OliveDrab1", "turquoise1", "tan1", "LightCyan2", "SeaGreen1", "PaleGreen2"]
           
MAX_TERRAIN_WIDGET_WIDTH = 5  # number of terrain widgets to be displayed horizontally before starting a new row
LABEL_FONT = ("Calibri", 10, "bold")
   
   
Settings = {    'map_width':    25,
                'map_height':   25,
           }
...
class TerrainWindow(tk.Frame):
    def __init__(self, outtxt, master=None, terrain_types=None):
    ...
    def create_terrain_widget(self, terrain_type, master):
        ...
        min_size_label = tk.Label(container, text="Min Size", font=LABEL_FONT)
        min_size_label.pack()
        min_size_scale = tk.Scale(container, from_=1, to=min(Settings["map_width"], Settings["map_height"]), 
                                  resolution=1, orient=tk.HORIZONTAL)
        min_size_scale.pack()
        
        max_size_label = tk.Label(container, text="Max Size", font=LABEL_FONT)
        max_size_label.pack()
        max_size_scale = tk.Scale(container, from_=1, to=min(Settings["map_width"], Settings["map_height"]), 
                                  resolution=1, orient=tk.HORIZONTAL)
        max_size_scale.pack()
        
        max_objects_label = tk.Label(container, text="Max Objects", font=LABEL_FONT)
        max_objects_label.pack()
        max_objects_scale = tk.Scale(container, from_=1, to=(Settings["map_width"] * Settings["map_height"] // 4), 
                                     resolution=1, orient=tk.HORIZONTAL)
    ...
    def generate_terrain(self):
        ...
        terrain_objects = []
        for i in range(5):  # just for testing -- sophisticated terrain gen is next goal
            ...
            terrain_objects.append(TerrainObject(ttype, locx, locy, size))
        ...
        self.outtxt.insert(tk.END, out_str)
...
def main():
    map = [[None for i in range(Settings['map_height'])] for j in range(Settings['map_width'])]
    ...
    outtxt.insert(tk.END, "Hello world")
    outtxt.config(state=tk.DISABLED)
    outtxt.pack()
    # outmsg = tk.Message(top)
    
    
    terrain_window = TerrainWindow(master=root, outtxt=outtxt, terrain_types=[pine_type, stone_type])
